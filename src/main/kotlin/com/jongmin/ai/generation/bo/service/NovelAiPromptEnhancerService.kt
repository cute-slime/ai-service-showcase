...
import com.jongmin.ai.generation.bo.dto.NovelAiGenerationSettingsResponse
import com.jongmin.ai.generation.bo.dto.NovelAiPromptEnhanceRequest
import com.jongmin.ai.generation.bo.dto.NovelAiPromptEnhanceResponse
import com.jongmin.ai.generation.bo.dto.NovelAiPromptTemplateBlockResponse
...
/**
 * NovelAI 프롬프트 인첸터 서비스
 *
 * 기존 캐릭터/배경 프롬프트를 기반으로 LLM 최적화를 수행하고,
 * 일관성 운영 규칙(고정 블록/가변 블록/생성 파라미터 권장값)을 함께 반환한다.
 */
@Transactional(propagation = Propagation.SUPPORTS, readOnly = true)
@Service
class NovelAiPromptEnhancerService(
  ...
  companion object {
    ...
    private const val DEFAULT_STYLE_BLOCK =
      "masterpiece, best quality, amazing quality, very aesthetic, anime coloring, cel shading, soft lighting"

    private const val DEFAULT_NEGATIVE =
      "lowres, bad anatomy, bad hands, missing fingers, extra digits, fewer digits, blurry, text, watermark, signature"

    private const val DEFAULT_BACKGROUND_BLOCK = "scenery, no humans"

    private val BACKGROUND_REQUIRED_NEGATIVE = listOf(
      "people", "person", "human", "face", "text", "watermark"
    )

    private val CHARACTER_REQUIRED_NEGATIVE = listOf(
      "bad anatomy", "bad hands", "extra fingers", "missing fingers", "text", "watermark"
    )

    private val SYSTEM_PROMPT = """
      |# Role
      |You are a NovelAI prompt enhancement specialist for production pipelines.
      |
      |# Objective
      |Improve the given base prompt while preserving style consistency across character/background assets.
      |
      |# Rules
      |1. Output Danbooru-like comma-separated tags.
      |2. Keep consistency by separating:
      |   - styleBlock (fixed for whole project)
      |   - subjectBlock (character or background identity)
      |   - variableSceneBlock (scene-only changes)
      |3. Respect locked template intent:
      |   - styleBlock should stay stable.
      |   - scene variation goes to variableSceneBlock.
      |4. If preferredArtistTags is provided, prioritize only that artist combination.
      |5. If assetKind is BACKGROUND, avoid person/human related positive tags.
      |6. Do not use weighted/emphasis syntax such as (tag:1.2), [tag], ((tag)).
      |7. Return concise and production-friendly output.
      |
  ...
  fun enhance(request: NovelAiPromptEnhanceRequest): NovelAiPromptEnhanceResponse {
    val normalizedRequest = request.copy(
      preferredArtistTags = normalizePreferredArtistTags(request.preferredArtistTags)
    )
    ...
    return NovelAiPromptEnhanceResponse(
      success = true,
  ...
  private fun normalizePreferredArtistTags(values: List<String>): List<String> {
    ...
    if (dedup.size < 2) {
      throw IllegalArgumentException("preferredArtistTags는 최소 2개 이상 입력해야 합니다.")
    }
    return dedup.values.toList()
  }

  private fun buildUserPrompt(request: NovelAiPromptEnhanceRequest): String {
    return buildString {
      appendLine("## Input")
  ...
  private fun callLlmWithFallback(
    request: NovelAiPromptEnhanceRequest,
    userPrompt: String,
  ): SystemChatResponse? {
    if (request.assistantId != null) {
      return runCatching {
        ...
        kLogger.warn { "[NovelAI Enhancer] assistantId=${request.assistantId} 호출 실패: ${e.message}" }
      }.getOrNull()
    }
    ...
    candidates.forEach { candidate ->
      val response = runCatching {
        ...
        kLogger.warn {
          "[NovelAI Enhancer] assistantType=${candidate.type}, category=${candidate.category} 호출 실패: ${e.message}"
        }
      }.getOrNull()
    ...
    kLogger.warn { "[NovelAI Enhancer] LLM 호출 실패, 규칙 기반 fallback 적용" }
    return null
  }
  ...
  private fun assistantCandidates(assetKind: NovelAiAssetKind): List<AssistantCandidate> {
    return when (assetKind) {
      NovelAiAssetKind.BACKGROUND -> listOf(
        AssistantCandidate(AiAssistantType.BACKGROUND_PROMPT, BACKGROUND_ASSISTANT_CATEGORY),
        AssistantCandidate(AiAssistantType.IMAGE_PROMPT_GENERATOR, null),
      )

      NovelAiAssetKind.CHARACTER -> listOf(
        AssistantCandidate(AiAssistantType.CHARACTER_PROMPT, CHARACTER_ASSISTANT_CATEGORY),
        AssistantCandidate(AiAssistantType.IMAGE_PROMPT_GENERATOR, null),
      )
    }
  }
  ...
  private fun parseLlmResult(response: SystemChatResponse?): ParsedLlmResult {
    val content = response?.content
    val assistantLabel = buildAssistantLogLabel(response)

    if (content.isNullOrBlank()) {
      kLogger.warn { "[NovelAI Enhancer] LLM 응답이 비어 있어 fallback 적용 - assistant=$assistantLabel" }
      return ParsedLlmResult(fallbackUsed = true)
    }
    ...
    if (json == null) {
      kLogger.warn {
        "[NovelAI Enhancer] LLM 응답에서 JSON 추출 실패, fallback 적용 - " +
  ...
  private fun normalizeResult(
    ...
    val styleTags = mergeTags(
      splitTags(styleSeed),
      request.preferredArtistTags.map { normalizeArtistTag(it) },
    ...
    val finalPromptTags = mergeTags(styleTags, subjectTags, variableTags)
    val finalPrompt = finalPromptTags.joinToString(", ")
  ...
  private fun buildSettings(request: NovelAiPromptEnhanceRequest): NovelAiGenerationSettingsResponse {
    ...
    return NovelAiGenerationSettingsResponse(
      sampler = locked?.sampler ?: "k_euler",
      steps = (locked?.steps ?: 28).coerceIn(12, 60),
      cfgScale = (locked?.cfgScale ?: 6.0).coerceIn(1.0, 15.0),
  ...
  private fun buildAppliedStrategies(
    ...
    val base = mutableListOf(
      ...
      "네거티브 프롬프트 고정 블록 유지",
      "샘플러/스텝/CFG/해상도 권장값 고정",
    )

    if (request.referencePrompts.isNotEmpty()) {
      base += "레퍼런스 프롬프트 기반 스타일 앵커링"
    }
    if (request.preferredArtistTags.isNotEmpty()) {
      base += "선호 작가 태그 조합 우선 적용"
    }
    if (request.sceneInstruction != null) {
      base += "장면 가변 블록 분리 적용"
    }

    return mergeTags(base, llmStrategies).map { it }
  }
  ...
  private fun normalizeArtistTag(raw: String): String {
    val tag = normalizeTag(raw)
    if (tag.isBlank()) return tag
    return if (tag.startsWith("{") || tag.startsWith("artist:", ignoreCase = true)) {
      tag
    } else {
      "{${tag}}"
    }
...
private data class NormalizedEnhancerResult(
