...
import com.jongmin.ai.role_playing.backoffice.dto.request.PatchPlace
import com.jongmin.ai.role_playing.backoffice.dto.response.BoPlaceItem
import com.jongmin.ai.role_playing.backoffice.service.BoPlaceService
...
@RestController
@RequestMapping("/v1.0")
class BoPlaceController(
  private val objectMapper: ObjectMapper,
  private val redisNodeRepository: RedisNodeRepository,
  private val boPlaceService: BoPlaceService
) : JController() {

  @PostMapping("/bo/places")
  fun create(@Valid @RequestBody dto: CreatePlace): BoPlaceItem {
    dto.accountId = session!!.accountId
    dto.ownerId = -1
    dto.currentVersion = 1.0f
    dto.version = mapOf()
    return boPlaceService.create(session!!, dto)
  }

  @GetMapping("/bo/places/{id}")
  fun findOne(@PathVariable(required = true) id: Long): BoPlaceItem {
    return boPlaceService.findById(id)
  }

  @GetMapping("/bo/places")
  fun findAll(
    ...
    return boPlaceService.findAll(statuses, q, jPageable(pageable))
  }

  @GetMapping("/bo/places/available")
  fun findAll(): List<BoPlaceItem> {
    return boPlaceService.findAll()
  }

  @PatchMapping("/bo/places")
  fun patch(@Valid @RequestBody dto: PatchPlace): Map<String, Any?> {
    val datum = objectMapper.convertValue(dto, object : TypeReference<MutableMap<String, Any>>() {})
    dto.type?.let { datum["type"] = it }
    return boPlaceService.patch(session!!, datum)
  }

  @DeleteMapping("/bo/places/{id}")
  fun delete(@PathVariable(required = true) id: Long): CommonDto.JApiResponse<Boolean> {
    return CommonDto.JApiResponse(data = boPlaceService.delete(session!!, id))
  }
}
